...
Here, The probability of making the shot doesn’t change between the two simulations. So, The distribution should be Somewhat similar to the first run since the shots are independent of each other.
...
There is an insufficient evidence that the hot hand model fits Kobe’s shooting patterns because the distibution of streak lengths are similar for the simulated shooter and for Kobe: both modes are 0 (more often then not there are 0 streaks) and both distributions are right skewed having similar ranges.
